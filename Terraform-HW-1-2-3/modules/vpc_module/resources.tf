...
resource "aws_vpc" "theVPC" {
  ...
  tags = {
	Name = "${var.name}VPC_${var.timeAdjustment}"
  }
}
...
resource "aws_internet_gateway" "GW" {
  ...
  tags = {
    Name = "${var.name}GW_${var.dateTimeCreation}"
  }
}
...
resource "aws_nat_gateway" "natGW" {
  ...
  tags = {
    Name = "${var.name}NAT_${var.dateTimeCreation}"
  }
}
...
resource "aws_subnet" "publicSubnet" {
	...
	tags = {
		Name = "${var.name}publicSubnet${count.index}_${var.dateTimeCreation}"
	}
}
...
resource "aws_subnet" "privateSubnet" {
	...
	tags = {
		Name = "${var.name}privateSubnet${count.index}_${var.dateTimeCreation}"
	}
}
...
resource "aws_route_table" "Public_RT" {
  ...
  tags = {
    Name = "${var.name}PublicRT_${var.dateTimeCreation}"
  }
}
...
resource "aws_route_table" "Private_RT" {
  ...
  tags = {
    Name = "${var.name}PrivateRT_${var.dateTimeCreation}"
  }
}
